        ...
        <div className="glass-panel border border-slate-200 rounded-xl overflow-hidden shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-850 text-sm text-slate-600">
                {products.map((product) => (
                  <tr key={product._id} className="hover:bg-slate-50/40 transition-colors">
                    {/* Image & Title */}
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 flex-shrink-0 rounded-lg bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center p-0.5">
                          {product.thumbnail?.url ? (
                            ...
                            <ShoppingBag size={18} className="text-slate-650" />
                          )}
                        </div>
                        <div className="max-w-[200px] sm:max-w-xs">
                          <h3 className="font-semibold text-slate-700 truncate leading-snug" title={product.name}>
                            {product.name}
                          </h3>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-slate-500 font-medium">{product.brand?.name || 'No Brand'}</span>
                            <span className="text-[9px] text-slate-600">•</span>
                            <span className="text-[10px] text-slate-500 font-medium">{product.category?.name || 'No Category'}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* SKU & Model */}
                    <td className="py-4 px-6">
                      <div className="text-xs text-slate-600 font-semibold tracking-wider">{product.sku}</div>
                      <div className="text-[11px] text-slate-550 mt-0.5 font-medium">{product.modelNumber}</div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      {product.discountPrice ? (
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-700">₹{product.discountPrice}</span>
